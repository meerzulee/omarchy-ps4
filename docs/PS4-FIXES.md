...
| Candidate | Select `wlan0` rather than the earlier `ap0`/`ap1` interfaces in the Quickshell network panel. Retain a disconnected Linux `wl*` station while NetworkManager reports mode `Unknown` after a saved connection is forgotten, so scanning can repopulate nearby networks. | `packages/omarchy-ps4/ps4-network-panel.patch`, `tests/network-panel-patch-test` |
