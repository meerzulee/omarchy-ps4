...
- On the local firmware-12.02 console, GoldHEN PayLoader was reachable and
  ready on TCP 9090. PSFree Enhanced embeds byte-identical v25 ELFs and can
  send them after GoldHEN is active; it does not itself supply a 12.02 browser
  exploit chain.
...
| Patch-based current LTS port | This project: Linux 6.18.44 + 47 active Baikal patches | Strict apply and container build; hardware pending | Test first because its inputs and provenance are owned here |
...
| Loader | PS4 Linux Loader v25 | Source audited and release hash verified; local boot pending | Use the pinned 1024MB ELF through GoldHEN PayLoader |
...
| A supported, reproducible Hyprland-on-PS4 product | This project has public Baikal B1 hardware evidence of Hyprland/OpenGL rendering on an experimental Linux 6.15 line, but not a clean 6.18 package, full integration test, or support contract. |
...
3. Test our pinned Linux 6.18.44 Baikal patch target without changing userspace; retain rmux 7.0.8 as a comparison tree.
